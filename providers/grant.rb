action :create do
  ...
  execute "#{new_resource.user}@#{new_resource.host}: GRANT #{new_resource.privileges}" do
    ...
    not_if 'mysql -r -B -e "SHOW GRANTS FOR \'' + new_resource.user + '\'@\'' + new_resource.host + '\';" | grep -F "GRANT ALL PRIVILEGES ON \`' + new_resource.database + '\`.' + new_resource.table + " TO '" + new_resource.user + "'@'" + new_resource.host + "'\""
  end
end
...
action :delete do
  ...
  execute "#{new_resource.user}@#{new_resource.host}: REVOKE #{new_resource.privileges}" do
    ...
    only_if 'mysql -r -B -e "SHOW GRANTS FOR \'' + new_resource.user + '\'@\'' + new_resource.host + '\';" | grep -F "GRANT ALL PRIVILEGES ON \`' + new_resource.database + '\`.' + new_resource.table + " TO '" + new_resource.user + "'@'" + new_resource.host + "'\""
  end
end
